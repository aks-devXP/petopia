      ...
      <nav className="h-full flex flex-col">
        <div className="p-4 pb-2 flex justify-end item-center">
          ...
          onClick={()=> expandHandler((prev)=>!prev)}
           className="p-1.5 rounded-lg bg-[#3C2A21] hover:text-white text-[#E5E5CB] 
           transition-colors">
...
export function SlidebarItem({icon, text, active, alert, click}){
  ...
  return(
    ...
    ${active? "bg-[#3C2A21] text-[#E5E5CB]" : "hover:text-white text-white/50"}`}>
      {icon}
      <span className={`overflow-hidden transition-all ${expanded? "w-40 ml-3": "w-0 m-0"}`}>{text}</span>
      {alert && (
        <div className={`ml-1 w-2 h-2 rounded bg-[#3C2A21] ${expanded? "": ""}`}/>
      )}

      {!expanded && (
        <div className={`absolute left-full rounded-md px-2 py-2 ml-5 bg-[#3C2A21] text-[#E5E5CB] text-sm
