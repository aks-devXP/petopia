...
/** interaction tuning */
const DECIDE = 8; // px threshold to decide rotate vs. ignore gesture
...
/** Desktop orbit control that just keeps camera target locked to pivot */
const DesktopControls = ({ pivot, min, max, zoomEnabled }) => {
  ...
  return (
    <OrbitControls
      ...
      makeDefault
      enablePan={false}
      enableRotate={false}        // rotation handled manually for “natural” feel
...
const ModelInner = ({
  ...
  const outer = useRef(null); // rotates/translates
  const inner = useRef(null); // contains the loaded model
  ...
  useLayoutEffect(() => {
    ...
    // Normalize model: center + uniform scale
    const sphere = new THREE.Box3().setFromObject(g).getBoundingSphere(new THREE.Sphere());
    const s = 1 / (sphere.radius * 2);
    g.position.set(-sphere.center.x, -sphere.center.y, -sphere.center.z);
    g.scale.setScalar(s);

    // Shadow flags + optional fade-in
    g.traverse((o) => {
      if (o.isMesh) {
        o.castShadow = true;
    ...
    // lock pivot & initial orientation
    g.getWorldPosition(pivotW.current);
    pivot.copy(pivotW.current);
    outer.current.rotation.set(initPitch, initYaw, 0);

    // auto frame perspective camera
  ...
  /** Hover parallax/rotation (desktop) */
  useEffect(() => {
    if (isTouch) return;
    const mm = (e) => {
  ...
  useFrame((_, dt) => {
    ...
    // place outer group at pivot + offsets (in NDC)
    const ndc = pivotW.current.clone().project(camera);
    ndc.x += cPar.current.x + (xOff || 0);
    ndc.y += cPar.current.y + (yOff || 0);
...
export default function ModelViewer({
  ...
  // Preload only for GLTF/GLB (safe no-op for other types)
  useEffect(() => {
    if (!url) return;
    if (/\.(glb|gltf)(\?|#|$)/i.test(url)) useGLTF.preload(url);
  }, [url]);
  ...
  const capture = () => {
    ...
    s.traverse((o) => {
      if (o.isLight && "castShadow" in o) {
        lights.push({ l: o, cast: o.castShadow });
        o.castShadow = false;
      }
    });
    ...
    lights.forEach(({ l, cast }) => (l.castShadow = cast));
  ...
  return (
    ...
    >
      ...
      <Canvas
        shadows
        frameloop="demand"
        gl={{ preserveDrawingBuffer: true, alpha: true }}         // transparent canvas
        onCreated={({ gl, scene, camera }) => {
          ...
          // make background transparent so it blends into the page
      ...
      >
        ...
        {/* extremely soft ground contact for grounding the asset */}
        <ContactShadows
          ref={contactRef}
          position={[0, -0.5, 0]}
          opacity={0.25}
